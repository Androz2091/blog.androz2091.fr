...
Bonjour à tous. Dans ce premier post sur ce blog, je vais détailler le reverse engineering de l'application PixPay, une néo-banque française.

Cet article me sert à la fois de notes dans le cas où j'aurais à faire un nouveau reverse engineering d'une application, et à la fois de tutoriel pour que vous puissiez (peut-être) apprendre quelques petites choses !

Dans ce post, j'utiliserai Charles Proxy, ProxyDroid, un Android 7.0 rooté (en fait un Samsung J3 2016 Android 5.0 avec une ROM Android 7.0) et Frida. 

## Reverse engineequoi ?

Le reverse engineering consiste à étudier la façon dont a été conçue une application, souvent pour pouvoir automatiser certaines actions de l'application avec du code. Un petit exemple parlant, si on comprend comment fonctionne Instagram, on peut parvenir à publier automatiquement des posts avec un script.

## Pourquoi avoir choisi PixPay ?

Après avoir déjà fait un peu de reverse engineering avec l'application Pronote, je voulais me familiariser un peu avec Charles Proxy et tous ces outils. Intercepter les requêtes de l'application n'aurait du me prendre qu'une petite heure mais comme vous allez le voir, cela s'est avéré un peu plus ardu. J'ai choisi PixPay par curiosité pour ces "néo-banques" et parce que c'est une des seules qui n'a pas déjà été étudiée.
...
* téléchargez Charles sur https://www.charlesproxy.com/
* lancez Charles
...
Merci d'avoir lu jusqu'ici et j'espère que ce post aura pu vous en apprendre un peu plus sur le reverse engineering, des applications Flutter notamment! Je remercie encore une fois [Kaki87](https://kaki87.net) qui m'a beaucoup aidé ainsi que l'auteur du script Frida, [Jeroen Beckers](https://blog.nviso.eu/author/dauntless/).
