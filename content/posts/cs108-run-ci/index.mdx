...
Ce cours de programmation orienté objet est en grande partie tourné vers les tests unitaires. Utiles pour tester l'état de son projet, nous avons appris jusqu'ici à les lancer manuellement de temps en temps.
...
* l'automatisation c'est généralement satisfaisant

CI/CD fait référence à intégration/développement continu (utilisé dans quasiment tous les gros projets). Bref, allons-y.
...
* téléchargez le fichier [run_tests.zip](./run_tests.zip)
* dézippez le fichier et déplacez-le dans un dossier `scripts`
* téléchargez le fichier [run_tests.yml](./run_tests.yml)
...
Pour régler cela, nous devons aussi fournir à `javac` les fichiers `.jar` compilés de Junit. Ils sont disponibles en ligne, je les ai placés dans un dossier `lib`.

Ainsi, la commande devient :

* `javac -d out/test/classes -classpath out/production/classes:./lib/org/junit/jupiter/junit-jupiter/5.10.2/junit-jupiter-5.10.2.jar:./lib/org/junit/jupiter/junit-jupiter-api/5.10.2/junit-jupiter-api-5.10.2.jar:./lib/org/opentest4j/opentest4j/1.3.0/opentest4j-1.3.0.jar:./lib/org/junit/platform/junit-platform-commons/1.10.2/junit-platform-commons-1.10.2.jar:./lib/org/apiguardian/apiguardian-api/1.1.2/apiguardian-api-1.1.2.jar:./lib/org/junit/jupiter/junit-jupiter-params/5.10.2/junit-jupiter-params-5.10.2.jar:./lib/org/junit/jupiter/junit-jupiter-engine/5.10.2/junit-jupiter-engine-5.10.2.jar:./lib/org/junit/platform/junit-platform-engine/1.10.2/junit-platform-engine-1.10.2.jar test/ch/epfl/chacun/*.java`

Félicitations, votre projet est compilé!

### Lancer les tests

Maintenant, nous devons lancer les tests via la console, sans passer par l'interface d'IntelliJ.

Pour cela, nous pouvons récupérer le fichier `junit-platform-console-standalone-1.10.2.jar` sur [ce site](https://repo1.maven.org/maven2/org/junit/platform/junit-platform-console-standalone/).
...
* l'étape `unzip library` permet d'extraire les fichiers Junit utiles pour compiler que j'ai préalablement téléchargés
